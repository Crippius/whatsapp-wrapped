...
class Plotter:
    ...
    def plot_emojis(
        self, who: str = "", reverse: bool = False, info: bool = True
    ) -> None:
        ...
        if info:
            txt = {
                "en": f"{sum(emojis.values())} total\nemojis 😯",
                "it": f"{sum(emojis.values())} emoji\ntotali 😯",
            }
            axlim = ax.get_xlim()
            ayl = ax.get_ylim()
            ax.text(
                x=axlim[0] + 1 if reverse else axlim[1] - 1,
                y=ayl[1] * 9 / 10,
                s=txt[self.lang],
                ha="left" if reverse else "right",
                va="top",
                fontsize=16,
        ...
        add_labels_to_bar(
            plot,
            emojis.values(),
            font_size=emojis_to_size_dict[len(emojis)],
            dir="vertical",
        )
        self.save_image()
    ...
    def plot_number_of_messages(self, interval: str = "day") -> None:
        ...
        if (
            (interval == "month" and diff > timedelta(days=360))
            or (interval == "week" and diff > timedelta(days=180))
            or (interval == "day" and diff > timedelta(days=30))
        ):
            plt.plot(x_pos, running_average, color="#075e55")
            if pd.to_datetime(x_pos[-1]) - pd.to_datetime(x_pos[0]) < timedelta(
                days=365
            ):
                if pd.to_datetime(x_pos[-1]) - pd.to_datetime(x_pos[0]) < timedelta(
                    days=365 // 2
        ...
        interval_dict = {
            ...
            "week": {"en": "week", "it": "settimana"},
            "month": {"en": "month", "it": "mese"},
            "year": {"en": "year", "it": "anno"},
        }
        title = {
            "en": f"Number of messages per {interval_dict[interval][self.lang]}",
    ...
    def plot_day_of_the_week(self) -> None:
        ...
        title = {
            "en": "Number of messages per weekday:",
    ...
    def plot_most_used_words(self, wordcloud: bool = True) -> None:
        ...
        if wordcloud:
            plot = WordCloud(
                ...
                mode="RGBA",
                colormap="summer",
            ).generate_from_frequencies(most_used_words)
            ...
            plt.bar(
                list(most_used_words.keys()), most_used_words.values(), color="#26d367"
            )
    ...
    def plot_most_active_people(self, group: bool = True, name: str = None) -> None:
        ...
        """
        people = {
            font_friendly(k): len(self.df[self.df.who == k]) for k in set(self.df.who)
        }
        ...
        if group:
            title = {
                "en": f"Most active people in {name}",
                "it": f"Persone più attive in {name}:",
            }
            plt.title(title[self.lang])
            set_to_font_size = {
                1: 150,
            ...
            plot = plt.barh(
                [str(i) for i in people.values()], people.values(), color="#26d367"
            )
            add_labels_to_bar(
                plot,
                people.keys(),
                font_size=set_to_font_size[len(people)],
                dir="horizontal",
                pos="internal",
            )
        else:
    ...
    def plot_time_of_messages(self) -> None:
        ...
        title = {
            "en": "Number of messages per time of day:",
        ...
        x_pos_fmt = [
            f"{'0' if i < timedelta(hours=10) else ''}{str(i)[:-3]}" for i in x_pos
        ]
